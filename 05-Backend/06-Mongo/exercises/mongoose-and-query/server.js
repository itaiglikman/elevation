...
const dbConnectionString = `mongodb+srv://itaig1998_db_user:<password>@cluster0.oypbsmo.mongodb.net/Query?retryWrites=true&w=majority&appName=Cluster0`;
...
async function ex3() {
    const people = await Person.find().and([
...
async function ex4() {
    // const people = await Person.count({'kids.hair': 'grey'})
    const people = await Person.count().and([
        { 'kids.hair': 'grey' },
        { kids: { $size: 2 } }
    ])
    console.log(people);
...
async function ex5() {
    const people = await Person.find({}, { weight: 1, kids: 1 }).and([
        { 'kids.weight': { $gt: 60 } },
        { weight: { $gt: 100 } }
    ])
    console.dir(people, { depth: null });
